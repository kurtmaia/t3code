/** ProviderDriver for the GitHub Copilot CLI's standard ACP server. */
...
export const CopilotDriver: ProviderDriver<CopilotSettings, CopilotDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const acpSettings = { binaryPath: effectiveConfig.binaryPath, apiEndpoint: "" };
      const adapter = yield* makeCursorAdapter(acpSettings, {
        provider: DRIVER_KIND,
        instanceId,
        environment: processEnv,
        makeAcpRuntime: makeCopilotAcpRuntime,
        ...(eventLoggers.native ? { nativeEventLogger: eventLoggers.native } : {}),
      });
      const textGeneration = yield* makeCursorTextGeneration(acpSettings, processEnv, {
        makeAcpRuntime: makeCopilotAcpRuntime,
      });

      const checkProvider = checkCopilotProviderStatus(effectiveConfig, processEnv, cwd).pipe(
